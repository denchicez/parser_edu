...
def is_site_correct(html_str, name,code1,code2):
    try:
...
def get_social_links(links):
    prefixes = prefixes = [['https://www.youtube.com/channel/','http://www.youtube.com/', 'https://www.youtube.com/user/','https://www.youtube.com/c/'], ['https://vk.com/'], ['https://www.instagram.com/']]
    youtube = []
    vk = []
    inst = []
    youtube_count=0
    ...
    if(estestv==1 or techno==1):
        ...
        if(site_is_work==True):
            ...
            if(title_org_site!='У сайта неизвестная кодировка' and title_org_site!='Не найдено'):
                ...
                podpis_youtube,podpis_vk,podpis_inst=social_links=get_social_links(get_links_from_page(url_name))
                if(podpis_youtube==0):
                    podpis_youtube='Нет аккаунта'
                if(podpis_vk==0):
                    podpis_vk='Нет аккаунта'
